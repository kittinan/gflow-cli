...
- Iconless buttons (e.g. "Add to Prompt") should be selected structurally, not by localized text — but **"the button that is not the icon button" is not the structural rule**, and on the migrated host that recipe is measured wrong twice over. `has_not=<i.google-symbols>` filters *nothing* there (the carrier tag is `mat-icon` — see [[ligature-carrier-differs-by-host]]), and the Frames popover's other visible button is `header-close-btn`, so the guess resolves to **Close**. Prefer the component's own class: the measured anchor for this exact button is `button.detail-add-to-prompt-btn`, confirmed on both entries into `flow-add-menu-popover-content` — the `@`-mention entry (`docs/superpowers/spikes/2026-09-05-migrated-r2v-attach-surface.md:72`) and the Frames entry (`docs/superpowers/spikes/2026-09-13-frames-picker-confirm.md`, #792).
