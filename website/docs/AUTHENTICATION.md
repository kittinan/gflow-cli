...
expires independently of, and much sooner than, the Google session — roughly a day. Re-reading
the session endpoint never moves its deadline; a lapsed session answers with a past `expires`,
`error: ACCESS_TOKEN_REFRESH_NEEDED`, and a stale `access_token` that every call rejects. When a
command finds that — or no `access_token` at all — it re-mints the Flow session once on its
own. It also does so at client start when the cached deadline (`.gflow_session_expires`) has
already passed, because cookie-authenticated calls would otherwise just 401. The re-mint is the
same round trip Flow's "Sign in" button performs, driven through NextAuth's API rather than the
page, so no click and no locale-dependent selector is involved:
...
3. The session is re-read, the new deadline is cached, and the command continues.
